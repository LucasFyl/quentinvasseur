...
$(document).ready(function(){
	...
	$('a.nextCol').on({
		mouseenter: function(){
			...
			$(this).empty();
			$(this).html('<p class="nextColFull">next collection</p>').promise().done(function(){
				TweenMax.to('p.nextColFull', 0.25, {top:0,opacity:1,ease:Power3.easeOut,delay:0.3})
			});
		}, mouseleave: function(){
			...
			setTimeout(function(){
				$('a.nextCol').html('<p class="justNext">next<p>').promise().done(function(){
					TweenMax.to('p.justNext', 0.25, {top:0,opacity:1,ease:Power3.easeOut});
				});
			}, 250);
		}
	});
	...
	function navNext() {
		...
		if ( hasPrev == false &&  i < imgLength-1) {
			...
			TweenMax.to('.img-wrap', 0.75, {top:slide,ease:Power2.easeOut,onComplete:function(){
				if ( hasPrev == true ) {
					$('.prev').removeClass('disabled');
				}
			}});
		} else if ( hasPrev == true &&  i < imgLength-1 ) {
			TweenMax.to('.img-wrap', 0.75, {top:slide,ease:Power2.easeOut});
			i ++;
		}
	...
	function superScroll(){
		TweenMax.to('.img-wrap', 1.8, {top:0,ease:Power2.easeOut});
		$('.next').removeClass('hidden');
		$('.superTop').addClass('hidden');	
		i = 0; hasPrev = false;
	}
